...
void Relatorio::gerarRelatorios(const Eleicao &eleicao, const Date &dataEleicao)
{
    ...
    std::sort(eleitos.begin(), eleitos.end(), [](const auto &a, const auto &b)
              { return *a > *b; });

    std::vector<std::shared_ptr<Candidato>> maisVotados = todosCandidatos;
    std::sort(maisVotados.begin(), maisVotados.end(), [](const auto &a, const auto &b)
              { return *a > *b; });

    std::sort(todosPartidos.begin(), todosPartidos.end(), [](const auto &a, const auto &b)
              { return *a > *b; });

    std::vector<std::shared_ptr<Partido>> partidosOrdenadosPorCandidato = todosPartidos;

    std::sort(partidosOrdenadosPorCandidato.begin(), partidosOrdenadosPorCandidato.end(), [](const auto &a, const auto &b)
              { 
        auto maisVotadoA = a->getCandidatoMaisVotado();
        auto maisVotadoB = b->getCandidatoMaisVotado();

        if (!maisVotadoA && !maisVotadoB) return a->getNumeroPartido() < b->getNumeroPartido();
        if (!maisVotadoA) return false;
        if (!maisVotadoB) return true;

        if (maisVotadoA->getVotosNominais() != maisVotadoB->getVotosNominais()) {
            return maisVotadoA->getVotosNominais() > maisVotadoB->getVotosNominais();
        }

        return a->getNumeroPartido() < b->getNumeroPartido(); });
    ...
    gerarRelatorioPrimeiroUltimo(partidosOrdenadosPorCandidato);
...
void Relatorio::gerarRelatorioPartidos(const std::vector<std::shared_ptr<Partido>> &partidos)
{
    ...
    for (const auto &p : partidos)
    {
        ...
        // LÓGICA DE PLURAL CORRIGIDA: Usa "voto" para 0 e 1, e "votos" para os demais.
        std::cout << pos++ << " - " << p->getSiglaPartido() << " - " << p->getNumeroPartido() << ", "
                  << votosTotais << " voto" << (votosTotais > 1 ? "s" : "")
                  << " (" << votosNominais << " " << (votosNominais <= 1 ? "nominal" : "nominais") << " e "
...
void Relatorio::gerarRelatorioPrimeiroUltimo(const std::vector<std::shared_ptr<Partido>> &partidos)
{
    ...
    for (const auto &p : partidos)
    {
        ...
        {
            std::cout << pos++ << " - " << p->getSiglaPartido() << " - " << p->getNumeroPartido() << ", "
                      << maisVotado->getNomeUrna() << " (" << std::to_string(maisVotado->getNumeroCandidato()) << ", "
                      << maisVotado->getVotosNominais() << (maisVotado->getVotosNominais() <= 1 ? " voto" : " votos") << ") / "
                      << menosVotado->getNomeUrna() << " (" << std::to_string(menosVotado->getNumeroCandidato()) << ", "
                      << menosVotado->getVotosNominais() << (menosVotado->getVotosNominais() <= 1 ? " voto" : " votos") << ")"
                      << std::endl;
        }
    }
}
...
void Relatorio::gerarRelatorioGenero(const std::vector<std::shared_ptr<Candidato>> &eleitos)
{
    if (eleitos.empty())
        return;
    long feminino = std::count_if(eleitos.begin(), eleitos.end(), [](const auto &c)
                                  { return c->getGenero() == Genero::FEMININO; });
    double totalEleitos = eleitos.size();
...
{
    int totalVotosValidos = std::accumulate(partidos.begin(), partidos.end(), 0, [](int sum, const auto &p)
                                            { return sum + p->getVotosTotaisDoPartido(); });
    int totalNominais = std::accumulate(partidos.begin(), partidos.end(), 0, [](int sum, const auto &p)
                                        { return sum + p->getVotosNominais(); });
